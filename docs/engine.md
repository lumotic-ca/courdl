...
Certificates expand to each `/learn/` course and download **one course at a time**. The parent folder uses the Coursera display name (company + Certificate or Specialization). After the last course, sibling folders are renamed `01 - Full Title` in catalog order. Keep `.cache/crawl.json` if you want to re-beautify without crawling again.
